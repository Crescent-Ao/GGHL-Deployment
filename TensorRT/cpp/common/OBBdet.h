...
inline float clamp(float input_value, float min_val, float max_val)
{
    if(input_value<min_val)
        return min_val;
    else if(input_value>max_val)
        return max_val;
    else
        return input_value;
}
void decode_infer(vector<float>& original_res, vector<Grid>& grid,vector<OBBInfo> &convert_res)
{
    int num_proposal = original_res.size()/25;//解析当前的anchor
    const int num_grid = grid.size();
    const int num_class = 15;
    for(int j=0;j<grid.size();j++)
    {
        int cur_pointer = 25 * j;
        OBBInfo cur_box;
        cur_box.x1 = grid[j].x * grid[j].stride + (grid[j].stride) / 2 - fast_exp(original_res[cur_pointer+3]) * grid[j].stride;
        cur_box.y1 = grid[j].y * grid[j].stride + (grid[j].stride) / 2 - fast_exp(original_res[cur_pointer + 0]) * grid[j].stride;
        cur_box.x2 = grid[j].x * grid[j].stride + (grid[j].stride) / 2 + fast_exp(original_res[cur_pointer + 1]) * grid[j].stride;
        cur_box.y2 = grid[j].y * grid[j].stride + (grid[j].stride) / 2 + fast_exp(original_res[cur_pointer + 2]) * grid[j].stride;
        cur_box.pred_r = sigmoid(original_res[cur_pointer + 8]);
        cur_box.conf = sigmoid(original_res[cur_pointer + 9]);
        vector<float> pred_s;
        if (cur_box.pred_r < 0.9)
        {
            for (int q = 4; q < 8; q++)
            {
                float cur_preds = clamp((double)sigmoid(original_res[cur_pointer + q]), 0.01, 1.0);
        ...
        for (int q = 10; q < 25; q++)
        {
            prob.push_back(sigmoid(original_res[cur_pointer + q]));
        }
        cur_box.prob.assign(prob.begin(),prob.end());
        convert_res.emplace_back(cur_box);
    }
}
void convert_result(vector<float> &original_res, vector<OBBInfo> &convert_res)
{
...
void convert_pred(vector<OBBInfo> &convert_result, const int test_size, const int ori_shape, const float conf_thresh, vector<OBBInfo8> &filter_Boxes)
{
    int org_h = ori_shape;
    int org_w = ori_shape;
...
void Convert_8Points(vector<OBBInfo8> &predict, vector<OBBInfo4> &BBox_4)
{
...
void non_max_supression(vector<OBBInfo8> &predict, float iou_thresh)
{
    ...
    Convert_8Points(predict, BBox_4);
    //保存下来的int数组就是原来的那个顺序
...
void non_max_supression_8_points(vector<OBBInfo8> &predict, float conf_thresh, float iou_thresh,
                                 vector<OBBInfo8> &BBox_filtered)
{
    ...
    for (int i = 0; i < predict.size(); i++)
    {
        ...
        predict[i].conf = cur_box.conf * max_prob<float>(cur_box.prob);
