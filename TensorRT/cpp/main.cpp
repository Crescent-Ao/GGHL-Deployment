...
static vector<int> strides{8,16,32};
...
void draw_objects(cv::Mat bgr, vector<OBBInfo8> objects)
{
    ...
    for (size_t i = 0; i < objects.size(); i++)
    {
        ...
        poly.push_back(cv::Point(obj.x1 , obj.y1));
        poly.push_back(cv::Point(obj.x2, obj.y2));
        poly.push_back(cv::Point(obj.x3, obj.y3));
        poly.push_back(cv::Point(obj.x4, obj.y4));
        cv::polylines(img, poly, true, color,4);
        cv::putText(img, text, cv::Point(obj.x1, obj.y1), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 255, 255));
    }
    cv::imwrite("/home/crescent/GGHL-Deployment/TensorRT/cpp/QAQ858.png", img);
}
class GGHLONNX
{
public:
...
void GGHLONNX::doInference(const cv::Mat &img)
{
    ...
    vector<Grid> grid;
    generate_grid_center_priors(input_H, input_W, strides, grid);
    cout << grid.size() << endl;
    cout << grid[2000].stride << endl;
...
DEFINE_string(image,"input_image files","QAQ");
int main(int argc, char *argv[])
{
    google::ParseCommandLineFlags(&argc, &argv, true);
    //这边进行一个初始化的过程
    const char *onnxfilename = "/home/crescent/GGHL_demo/GGHL.onnx";
    string engine_file = "/home/crescent/GGHL_dec.engine";
